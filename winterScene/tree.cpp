...
void tree::drawBranch(int rotate, int color, GLuint texID, GLuint transID, GLuint colorID)
{
	...
	if (rotate == 1)
	{
		transform = glm::rotate(transform, -45.f, glm::vec3(0, 1, 0));
	}
	else if (rotate == 2)
	{
		transform = glm::rotate(transform, 45.f, glm::vec3(0, 1, 0));
	...
	{
		transform = glm::rotate(transform, 45.f, glm::vec3(1, 0, 0));
	}
	else if (rotate == 4)
	{
		transform = glm::rotate(transform, -45.f, glm::vec3(1, 0, 0));
	...
	glDisableVertexAttribArray(0);
	glDisableVertexAttribArray(2);
	glUniformMatrix4fv(transID, 1, GL_FALSE, &(transform[0][0]));
