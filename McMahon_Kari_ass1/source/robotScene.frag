...
void main()
{
	...
	vec3 N = normalize(normalmatrix * normals);		//normal
    vec3 L = lightpos.xyz - P.xyz;	// light direction
	float distancetolight = length(L);	
	L = normalize(L);
	vec3 D = normalize(vec3(0,0,1)); //spotlight direction
	vec3 ambient = diffuse_albedo.xyz * 0.2;
	
	//spotlight code - does not work correctly - see report
	...
	{
		outputColor = vec4(ambient, 1.0); //ambient color if not within cone direction
	}
